...
from shared_memory import (
    SharedMemory,
    SharedMemoryUpdater,
    SINGLE_QUERY_CRITIQUE_WITH_MEMORY_TEMPLATE,
)
...
class Config:
    ...
    local_sampling_backend: str = "transformers"
    local_vllm_device: str = "cuda:1"
    ...
    shared_memory_max_bullets: int = 50
    shared_memory_max_new_per_step: int = 3
    shared_memory_in_prompt: bool = False  # include shared memory in problem-solving prompt
...
Additionally, consider these globally useful insights that apply to all math problems:
{shared_memory}"""
...
class PrincipleUpdater:
    def __init__(self, renderer, sampling_client, sampling_params, max_operations=1, shared_memory=None):
        self.renderer = renderer
        self.sampling_client = sampling_client
        self.sampling_params = sampling_params
        self.max_operations = max_operations
        self.shared_memory = shared_memory  # SharedMemory | None
    ...
    def _batch_update(self, principles, critiques, save_dir, max_retries=3):
        ...
        # if shared memory is available, include it so the LLM avoids duplicating global insights
        if self.shared_memory is not None and len(self.shared_memory) > 0:
            revision_prompt = SINGLE_QUERY_CRITIQUE_WITH_MEMORY_TEMPLATE.format(
                shared_memory=self.shared_memory.get_context_string(),
...
def main(config: Config):
    ...
    if config.test_dataset_size > 0:
        test_data = test_data[: config.test_dataset_size]
    ...
    if config.use_local_backend:
        ...
        service_client = LocalServiceClient(
            ...
            sampling_backend=config.local_sampling_backend,
            vllm_device=config.local_vllm_device,
    ...
    shared_memory = SharedMemory(max_bullets=config.shared_memory_max_bullets) if config.enable_shared_memory else None
    if shared_memory is not None:
        logger.info(f"SharedMemory enabled (max_bullets={config.shared_memory_max_bullets})")
    ...
    effective_lora_rank = config.local_lora_rank if config.use_local_backend else config.lora_rank
    ...
    for epoch in range(config.n_epochs):
        ...
        for batch_idx in range(num_batches):
            ...
            if step > 0:
                ...
                # Restore shared memory if enabled
                if shared_memory is not None:
                    sm_filename = os.path.join("data", config.domain, "train", config.experiment_name, f"step_{step}/shared_memory.json")
                    if os.path.exists(sm_filename):
                        sm_state = json.load(open(sm_filename))
                        shared_memory.load_state_dict(sm_state)
                        print(f"Loaded SharedMemory: {len(shared_memory)} bullets")
            ...
            if shared_memory is not None and config.shared_memory_in_prompt and len(shared_memory) > 0:
                test_batch_data = [{
                    "prompt": PROBLEM_WITH_PRINCIPLE_AND_MEMORY_TEMPLATE.format(
            ...
            for program_idx in range(config.num_parallel_programs):
                ...
                # Format the batch data with principles (+ shared memory if enabled)
                formatted_principles_i = "\n".join([f"[{i}]. {e}" for i, e in principles.items()])
                if shared_memory is not None and config.shared_memory_in_prompt and len(shared_memory) > 0 and principles:
            ...
            if "evolution" in config.train_mode:
                ...
                # Self reflection (short-term, per-strategy — ReflectivePrompt analogy)
                ...
                try:
                    principle_updater = PrincipleUpdater(
                        ...
                        shared_memory=shared_memory,  # inject global memory as context
                    )
                ...
                # Shared Memory update (long-term, cross-strategy — ACE/ReflectivePrompt long-term reflection)
                if shared_memory is not None:
                    print("Updating SharedMemory ...")
                    sm_updater = SharedMemoryUpdater(
                        renderer=renderer,
                        sampling_client=evolution_sampling_server,
                        sampling_params=sampling_params,
                        max_new_bullets=config.shared_memory_max_new_per_step,
                    )
                    sm_updater.update(
                        rollouts_all_programs=rollouts_all_programs,
                        program_lists=program_lists,
                        shared_memory=shared_memory,
                        step=step,
                        save_dir=cur_step_dir,
                    )
                    print(f"SharedMemory: {len(shared_memory)} bullets")
            ...
            # save new evolution pool (and shared memory if enabled)
            ...
            if shared_memory is not None:
                sm_filename = os.path.join(evolution_dir_path, "shared_memory.json")
                with open(sm_filename, "w") as sm_file:
                    json.dump(shared_memory.state_dict(), sm_file, indent=2)
